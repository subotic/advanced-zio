...
package payments {
   ...
   * EXERCISE
   *
   * Create a test implementation of `PaymentProcessor`, together with a
   * layer to describe its creation.
   */
  final case class PaymentProcessorTest(acceptors: Ref[List[Acceptor]], history: Ref[List[(PaymentMethod, Double)]])
...
package db {
   ...
   * EXERCISE
   *
   * Create a test implementation of `Database`, together with a
   * layer to describe its creation.
   */
  final case class DatabaseTest(config: DbConfig, ref: Ref[Map[String, Either[IOException, ResultSet]]])
      extends Database {
...
package comms {
   ...
   * EXERCISE
   *
   * Create a test implementation of `Emailer`, together with a
   * layer to describe its creation.
   */
  final case class EmailerTest(ref: Ref[List[(String, String, String, String)]]) extends Emailer {
    def sendEmail(subject: String, body: String, to: String, from: String): Task[Unit] =
      ref.update(_ :+ (subject, body, to, from)).unit
  }
  object EmailerTest {
    val layer: ZLayer[Any, Nothing, Emailer] =
      ZLayer {
        for {
          ref <- Ref.make(List.empty[(String, String, String, String)])
...
object core {
  ...
  final case class OrderProcessLive(processor: PaymentProcessor, orderRepo: OrderRepo, emailer: Emailer)
